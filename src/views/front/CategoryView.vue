...
const getProducts = (page = 1) => {
  ...
  axios.get(`${VITE_APP_URL}/api/${VITE_APP_PATH}/products/?page=${page}&category=${category}`)
    .then((res) => {
...
const isFavorite = (product) => {
  return useFavoritesStore.isFavorite(product)
}
const toggleFavorite = (product) => {
  useFavoritesStore.toggleFavorite(product)
}
const removeFavorite = (product) => {
  useFavoritesStore.removeFavorite(product)
}
onMounted(() => {
  getProducts()
})
